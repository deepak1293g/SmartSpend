            ...
            if (authView === 'signup') {
                ...
                const { error, data } = await supabase.auth.signUp({
                    ...
                    options: {
                        data: {
                            phone: fullPhone,
                            full_name: signupName.trim() || email.split('@')[0]
                        }
                    }
                });
                ...
                <div className="glass rounded-[2rem] md:rounded-[3rem] p-1 shadow-[0_0_80px_rgba(79,70,229,0.15)] border-white/10 overflow-hidden">
                    <div className="bg-slate-900/40 backdrop-blur-3xl rounded-[1.9rem] md:rounded-[2.9rem] p-6 md:p-14">
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); handleAuth(); }} className="space-y-5 md:space-y-6">
                            <div className="group">
                                <label className={`block text-[9px] md:text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 px-1 transition-colors ${authView === 'signin' ? 'group-focus-within:text-indigo-400' : 'group-focus-within:text-emerald-400'}`}>Email</label>
                                <input
                                    type="email"
                                    required
                                    value={email}
                                    onChange={(e) => setEmail(e.target.value)}
                                    className={`w-full bg-slate-950/40 border border-white/5 rounded-2xl px-4 md:px-6 py-2.5 md:py-4 text-xs md:text-base text-white focus:outline-none transition-all placeholder:text-slate-800 ${authView === 'signin' ? 'focus:border-indigo-500/50' : 'focus:border-emerald-500/50'}`}
                                    placeholder="operator@matrix.net"
                                />
                            </div>
                            <div className="group">
                                <label className={`block text-[9px] md:text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 px-1 transition-colors ${authView === 'signin' ? 'group-focus-within:text-indigo-400' : 'group-focus-within:text-emerald-400'}`}>Password </label>
                                <div className="relative">
                                    <input
                                        ...
                                        className={`w-full bg-slate-950/40 border border-white/5 rounded-2xl px-4 md:px-6 py-2.5 md:py-4 pr-10 md:pr-14 text-xs md:text-base text-white focus:outline-none transition-all placeholder:text-slate-800 ${authView === 'signin' ? 'focus:border-indigo-500/50' : 'focus:border-emerald-500/50'}`}
                                        placeholder="••••••••"
                                    />
                                    <button
                                        type="button"
                                        onClick={() => setShowPassword(!showPassword)}
                                        className={`absolute right-4 top-1/2 -translate-y-1/2 p-1.5 text-slate-600 transition-colors ${authView === 'signin' ? 'hover:text-indigo-400' : 'hover:text-emerald-400'}`}
                                    >
                                        {showPassword ? (
