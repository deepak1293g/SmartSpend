        ...
        {toast && (
          ...
          >
            <div className={`glass p-4 rounded-2xl border ${toast.type === 'error' ? 'border-rose-500/30' : 'border-emerald-500/30'} flex items-center gap-4 shadow-2xl backdrop-blur-3xl bg-slate-900/60`}>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${toast.type === 'error' ? 'bg-rose-500/20 text-rose-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
                {toast.type === 'error' ? (
              ...
              <p className="text-xs font-black uppercase tracking-widest italic whitespace-nowrap overflow-hidden text-ellipsis">{toast.message}</p>
